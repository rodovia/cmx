...
struct S_MEMORY_BUCKET *create_bucket(size_t size) {
  ...
  struct S_MEMORY_BUCKET *bucket = (struct S_MEMORY_BUCKET *) mmap(size / PAGE_SIZE);
  if (bucket != NULL) {
